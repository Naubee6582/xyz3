...
void draw(){
  switch (gameState){
   ...
   if ( mouseX >= ix && mouseX <= ix+sideLength && 
     mouseY >= iy && mouseY <= iy+sideLength){
     int col = int((mouseX-ix) / (sideLength/nSlot));
     int row = int((mouseY-iy) / (sideLength/nSlot));
     if (col<=3 && row<=3){
...
int countNeighborBombs(int col,int row){
  ...
  int count=0;
    ...
    for(int j = -1;j <= 1;j++){
      ...
      int neightCol = col+j;
      int neightRow = row+k;
      if (0<=neightCol && neightCol<=3 &&
          0<=neightRow && neightRow<=3){
         if(slot[neightCol][neightRow]==SLOT_BOMB||
            slot[neightCol][neightRow]==SLOT_DEAD||
            slot[neightCol][neightRow]==SLOT_FLAG_BOMB){
           count+=1;
      }
    }
  }
...
void mousePressed(){
  if ( gameState == GAME_RUN &&
    ...
    if (mouseButton == LEFT){ // left click
      ...
      if(slot[col][row] == SLOT_OFF && flagCount < bombCount){
